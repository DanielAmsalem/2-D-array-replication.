...
def main(import_export: IMPORT_EXPORT, run_name, mean_Cg, is_resumed=False) -> None:
    ...
    # -------------------------------------------------------------
    # DYNAMIC LOOP COUNT FIX:
    # Ensures the memory structures expand to fit arbitrarily high
    # task array indices if you decide to add more tasks later!
    # -------------------------------------------------------------
    loop_count = max(960, END_LOOP_IDX)
